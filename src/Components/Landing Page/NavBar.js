...
const Navbar = () => {
  ...
  const navItemsDesktopClassNames =
    'text-white text-[2.8rem] hover:text-gray-300 leading-[1] cursor-pointer'
    ...
    <nav className='bg-[#0A265F] text-white py-[2.1rem] border-b-[.1rem] border-[#ffffff26] z-50 top-0 sticky md:relative'>
      <div className='container flex items-center justify-between'>
        ...
        <div className='md:hidden relative z-50'>
          ...
          <div
            className={clsx(
              'bg-[#0A265F] py-4 text-center flex flex-col justify-between fixed top-0 right-0 z-50 h-full max-w-[300px]',
              styles.mobileMenu,
              {
          ...
          >
            <div>
              <div
                onClick={handleMobileMenuToggle}
                className='flex items-center p-[32px] cursor-pointer'
              >
                <ArrowLeftIcon className={styles.arrowIcon} />
                <span className='font-medium leading-[1] text-[4rem] ml-[20px]'>
                  Continue Exploring
                </span>
              </div>
        ...
        <div
          onClick={handleMobileMenuToggle}
          className={clsx(styles.mobileBack, {
        ...
        <div className='hidden md:flex items-center space-x-14 lg:space-x-28 text-arkiterWhite-100 translate-y-[-7%]'>
          {anchorLinks.map(({ href, id, text }) => (
            <a key={id} href={href} className={navItemsDesktopClassNames}>
              {text}
            </a>
          ))}
        </div>
